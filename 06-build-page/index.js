...
async function build() {
    ...
    for (let compFile of components) {
        const stats = await fsPromises.stat(path.join(__dirname, 'components', compFile))
        if (stats.isFile() && path.extname(compFile) === '.html') {
            const component = path.basename(compFile, path.extname(compFile));
            const content = await fsPromises.readFile(path.join(componentsPath, compFile))
...
async function copyDir(assets, destAssets) {
    ...
    for (let file of inner) {
        ...
        if (stats.isFile()) {
            await fsPromises.copyFile(path.join(assets, file), path.join(destAssets, file))
        } else {
